...
class Authentication(object):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        # encontro token en la respuesta
        if user is not None:
            if type(user) == str:
        ...
        response = Response({'error':'No han enviado las credenciales', 'expired':self.user_token_expired}, status = status.HTTP_400_BAD_REQUEST)  
        response.accepted_renderer = JSONRenderer()
        response.accepted_media_type = 'application/json'
        response.renderer_context = {}
        return response